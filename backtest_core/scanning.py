...
def scan_polyfit_ma_stoploss_nextday_guard_parameters(
    base_data: pd.DataFrame,
    param_space: dict[str, list],
    max_evals: int = 800,
    random_seed: int = 42,
    ...
    for i, params in enumerate(selected, start=1):
        ...
        if i % 200 == 0 or i == len(selected):
            elapsed = time.time() - start
            print(f"止损次日偏离度保护策略训练集扫描进度: {i}/{len(selected)}，耗时 {elapsed:.1f}s")

    result_df = pd.DataFrame(results)
    if result_df.empty:
        raise ValueError("止损次日偏离度保护策略训练参数扫描结果为空，请检查参数范围")
