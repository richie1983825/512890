...
from .scanning import scan_ma_parameters, scan_parameters
...
def run_walk_forward_validation_3y1y(
    ...
    for window_idx, (train_start, train_end, val_start, val_end) in enumerate(build_rolling_splits_3y1y(base_data), start=1):
        ...
        if generate_artifacts:
            ...
            if print_daily:
                print_daily_cumulative_returns_with_signals(daily_df, val_stats["_trades"], label=f"3年训练1年验证窗口{window_idx}", max_rows=daily_max_rows)
...
def run_ma_walk_forward_validation_3y1y(
    ...
    for window_idx, (train_start, train_end, val_start, val_end) in enumerate(build_rolling_splits_3y1y(base_data), start=1):
        ...
        if generate_artifacts:
            ...
            if print_daily:
                print_daily_cumulative_returns_with_signals(daily_df, stats["_trades"], label=f"MA{int(ma_window_days)} 窗口{window_idx}", max_rows=daily_max_rows)
            annual_df.to_csv(annual_csv, index=True, encoding="utf-8-sig")
            daily_df.to_csv(daily_csv, index=True, encoding="utf-8-sig")
            scan_df.head(50).to_csv(scan_csv, index=False, encoding="utf-8-sig")
            export_trade_records_csv(
...
def run_polyfit_walk_forward_validation_3y1y(
    ...
    for window_idx, (train_start, train_end, val_start, val_end) in enumerate(build_rolling_splits_3y1y(base_data), start=1):
        ...
        if generate_artifacts:
            ...
            if print_daily:
                print_daily_cumulative_returns_with_signals(daily_df, stats["_trades"], label=f"Polyfit拟合窗{int(fit_window_days)} 窗口{window_idx}", max_rows=daily_max_rows)
            annual_df.to_csv(annual_csv, index=True, encoding="utf-8-sig")
            daily_df.to_csv(daily_csv, index=True, encoding="utf-8-sig")
            scan_df.head(50).to_csv(scan_csv, index=False, encoding="utf-8-sig")
            export_trade_records_csv(
